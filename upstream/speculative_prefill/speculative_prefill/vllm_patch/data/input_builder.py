...
class AugmentedModelInputForGPUBuilder(ModelInputForGPUBuilder):
    def _compute_lens(
        ...
        is_encoder_decoder = getattr(
            ...
            "is_encoder_decoder",
            getattr(self.runner.model_config, "is_encoder_decoder_model", False),
        )
        ...
        if hasattr(inter_data, "prompt_lens"):
            inter_data.prompt_lens[seq_idx] = seq_data.get_prompt_len()
        inter_data.context_lens[seq_idx] = context_len
        inter_data.query_lens[seq_idx] = seq_len - context_len
        if hasattr(inter_data, "inputs_embeds"):
            inter_data.inputs_embeds = None
        if hasattr(inter_data, "token_types"):
