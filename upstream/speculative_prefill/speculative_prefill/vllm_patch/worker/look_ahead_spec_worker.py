...
def _last_prefill_queries(q: torch.Tensor, attn_metadata: Any) -> torch.Tensor:
    prefill_metadata = getattr(attn_metadata, "prefill_metadata", None)
    ...
    query_start_loc = getattr(prefill_metadata, "query_start_loc", None)
    if query_start_loc is not None:
        return q[query_start_loc[1:].long() - 1, :]

    seq_lens_tensor = getattr(prefill_metadata, "seq_lens_tensor", None)
    if seq_lens_tensor is not None:
        return q[torch.cumsum(seq_lens_tensor, dim=0).long() - 1, :]

    return q

def _forward_with_query_dump(
    self: Union[LlamaAttention, Qwen3Attention],
    ...
    query_buffer[layer_idx].append(_last_prefill_queries(q, attn_metadata))
...
class LookAheadSpecWorker(Worker):
    def load_model(self):
        self._load_prefixed_model()
        if not isinstance(self.model_runner.model,
                          (LlamaForCausalLM, Qwen3ForCausalLM)):
            raise NotImplementedError(
                f"Unsupported draft model type: {type(self.model_runner.model)}")
        ...
        for layer_idx, layer in enumerate(self._decoder_layers()):
            if isinstance(layer, (LlamaDecoderLayer, Qwen3DecoderLayer)):
                assert isinstance(layer.self_attn, (LlamaAttention, Qwen3Attention))
            else:
                raise NotImplementedError(
                    f"Unsupported draft layer type: {type(layer)}")
        ...
        config = self.model_runner.model.config
    ...
    def _load_prefixed_model(self):
        if self.model_runner.lora_config:
            raise NotImplementedError("SpecPrefill draft LoRA is not supported.")
        if self.model_runner.prompt_adapter_config:
            raise NotImplementedError(
                "SpecPrefill draft prompt adapters are not supported.")

        logger.info("Starting to load draft model %s...",
                    self.model_runner.model_config.model)
        loader = get_model_loader(self.vllm_config.load_config)
        target_device = torch.device(self.vllm_config.device_config.device)
            ...
            with set_default_torch_dtype(self.model_config.dtype):
                with target_device:
                    model = initialize_model(
                        ...
                        prefix="spec_prefill_draft",
                    )
                loader.load_weights(model, self.model_config)
                process_weights_after_loading(model, self.model_config,
                                              target_device)
            time_after_load = time.perf_counter()

        self.model_runner.model = model.eval()
        self.model_runner.model_memory_usage = m.consumed_memory
        logger.info("Draft model loading took %.4f GiB and %.6f seconds",
                    m.consumed_memory / GiB_bytes,
                    time_after_load - time_before_load)

    def _decoder_layers(self):
        return self.model_runner.model.model.layers
