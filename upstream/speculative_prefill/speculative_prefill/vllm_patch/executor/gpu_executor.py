...
class PatchedGPUExecutor(GPUExecutor):
    def _get_worker_module_and_class(self) -> Tuple[str, str, Optional[Callable[[], Type[WorkerBase]]]]:
        ...
        pruning_mode = os.environ.get("PREFILL_PRUNING_MODE", None)
        if pruning_mode == "embedding_norm":
            worker_module_name = "speculative_prefill.vllm_patch.worker.embedding_norm_worker"
            worker_class_name = "create_embedding_norm_worker"
        elif pruning_mode == "middle_layer_norm":
            worker_module_name = "speculative_prefill.vllm_patch.worker.middle_layer_norm_worker"
            worker_class_name = "create_middle_layer_norm_worker"
        elif os.environ.get("SPEC_MODEL", None):
