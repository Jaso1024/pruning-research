...
METHOD_NOTES = {
    "tome_fixed": (
        ...
        "size-weighted averaging. Fixed schedule: request the same merge_r at every ViT block. "
        "Dense-task adaptation: source maps restore the full patch grid before the Depth Anything depth head."
    ),
    "evit_fixed": (
        ...
        "MLP. Fixed schedule: the inattentive fused-token budget is chosen to remove merge_r "
        "tokens per block. Dense-task adaptation keeps source maps instead of permanently dropping grid cells."
    ),
    "ats_fixed": (
        "ATS-inspired deterministic fixed schedule: significance is derived from class-token "
        "attention and value/token magnitude. Unlike original ATS, this uses exactly merge_r "
        "as the requested per-block merge budget instead of a cumulative attention-mass threshold."
    ),
    "token_pooling_fixed": (
        ...
        "tokens. This fixed-schedule evaluator uses a deterministic greedy agglomerative path "
        "with merge_r requested at every block, then source maps restore the dense patch grid."
    ),
    "pitome_fixed": (
        ...
        "bipartite merging. Fixed schedule: request merge_r candidates at every block."
    ),
    "adamerge_fixed": (
        "AdaMerge near-faithful adaptation: salience is estimated by column-wise feature-affinity "
        "centrality and used in the merge score/weighted average. Fixed schedule: the paper's "
        "adaptive per-layer merge intensity is disabled and merge_r is requested at every block."
    ),
    "mctf_fixed": (
        "MCTF-inspired fixed schedule: merge candidates are scored by token similarity while "
        "penalizing high-salience and large-source tokens. This uses merge_r at every block and "
        "restores the full dense grid before the Depth Anything head."
    ),
    "ppt_fixed": (
        ...
        "Fixed schedule: request merge_r total reductions at every block."
    ),
}
...
def require_ready(dataset_root: Path, checkpoint: Path, encoder: str) -> None:
    ...
    if not (dataset_root / "annotations.json").is_file():
        raise FileNotFoundError(f"DA-2K annotations not found under: {dataset_root}")
...
def _ats_fixed_reduce(
    ...
    *,
    ...
    if merge_r <= 0 or patch_count < 2:
        return x, sizes, sources, 0
    keep_count = max(1, patch_count - min(merge_r, patch_count - 1))
    keep_mask = torch.zeros(patch_count, dtype=torch.bool, device=x.device)
    keep_mask[torch.topk(stats.significance, k=keep_count, largest=True).indices] = True
    return _merge_unselected_to_anchors(
        x,
        sizes,
        sources,
        keep_mask,
...
def _mctf_reduce(
    ...
    *,
    ...
    if merge_r <= 0 or patch_count < 2:
        return x, sizes, sources, 0
    score = stats.similarity
    score = score - config.salience_lambda * (stats.significance[:, None] + stats.significance[None, :]) * 0.5
    score = score - config.size_lambda * (stats.size[:, None] + stats.size[None, :]) * 0.5
    allowed = torch.ones(patch_count, dtype=torch.bool, device=x.device)
    pairs = _greedy_pairs(
        score,
        merge_r=merge_r,
        allowed_source=allowed,
        allowed_dst=allowed,
        source_preference=stats.significance + config.size_lambda * stats.size,
    )
    return _merge_local_pairs(x, sizes, sources, pairs, special_count=special_count)
...
def _run_actual_block(
    ...
    *,
    ...
    elif config.method == "ats_fixed":
        x, sizes, sources, merged = _ats_fixed_reduce(
            x,
            sizes,
            sources,
            stats,
    ...
    elif config.method == "token_pooling_fixed":
    ...
    elif config.method == "pitome_fixed":
    ...
    elif config.method == "adamerge_fixed":
    ...
    elif config.method == "mctf_fixed":
        x, sizes, sources, merged = _mctf_reduce(
            x,
            sizes,
            sources,
            stats,
...
def evaluate(config: ActualTokenConfig) -> dict[str, Any]:
    ...
    calib_stats = None
    calib_status = "disabled_fixed_schedule"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-json", type=Path, default=Path("eval_outputs/fixed_token_methods/tome_fixed_r57_full.json"))
    parser.add_argument("--encoder", choices=sorted(MODEL_CONFIGS), default="vits")
    parser.add_argument("--input-size", type=int, default=518)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--method", choices=METHODS, default="tome_fixed")
