...
def collect_da2k_images(dataset_root: Path, max_images: int, scene_type: str) -> list[Path]:
    ...
    paths = []
    for relative_path in annotations:
...
class ExpComboStats:
    def __init__(self, betas: list[float], heads: int, ridge: float) -> None:
        self.betas = list(betas)
        self.heads = heads
        self.ridge = ridge
        basis = len(self.betas)
        self.gram = np.zeros((heads, basis, basis), dtype=np.float64)
        self.rhs = np.zeros((heads, basis), dtype=np.float64)
    ...
    def update(self, source: np.ndarray, target: np.ndarray) -> None:
        ...
        for i, left in enumerate(basis_values):
            self.rhs[:, i] += np.einsum("bhnd,bhnd->h", left, target, optimize=True)
            for j in range(i, len(basis_values)):
                dot = np.einsum("bhnd,bhnd->h", left, basis_values[j], optimize=True)
                self.gram[:, i, j] += dot
                if i != j:
                    self.gram[:, j, i] += dot
    ...
    def best_by_head(self) -> list[dict]:
        results = []
        eye = np.eye(len(self.betas), dtype=np.float64)
        for head in range(self.heads):
            gram = self.gram[head]
            rhs = self.rhs[head]
            if self.ridge > 0:
                scale = np.trace(gram) / max(len(self.betas), 1)
                gram = gram + eye * self.ridge * max(scale, 1e-12)
            try:
            ...
            sse = target_ss - 2.0 * float(coeffs @ rhs) + float(coeffs @ self.gram[head] @ coeffs)
            mse = max(0.0, sse / max(count, 1))
            target_mse0 = target_ss / max(count, 1)
            rel_mse = mse / target_mse0 if target_mse0 > 0.0 else 0.0
            fitted_ss = float(coeffs @ self.gram[head] @ coeffs)
            cosine = 0.0
            if fitted_ss > 0.0 and target_ss > 0.0:
                cosine = float(coeffs @ rhs) / float(np.sqrt(fitted_ss * target_ss))
            results.append(
                {
                    "head": head,
                    "betas": self.betas,
...
def fit(args: argparse.Namespace) -> dict:
    ...
    betas = parse_floats(args.basis_betas)
    ...
    stats = {
        (source_name, block_index): ExpComboStats(betas, heads, args.ridge)
        for source_name in sources
        for block_index in range(len(blocks))
    }
    global_stats = {source_name: ExpComboStats(betas, heads, args.ridge) for source_name in sources}
    ...
    for source_name in sources:
        ...
        all_heads = []
        for block_index in range(len(blocks)):
            head_results = stats[(source_name, block_index)].best_by_head()
            all_heads.extend(head_results)
        ...
        global_heads = global_stats[source_name].best_by_head()
        results[source_name] = {
            "global_by_head": {
                "summary": summarize_heads(global_heads),
                "heads": global_heads,
            },
            "all_block_heads_summary": summarize_heads(all_heads),
            "blocks": blocks_result,
        }

    result = {
        "metadata": {
            "fit_type": "multi_exp_head_ema",
            ...
            "note": "Each head is fit as sum_k coeff_k * causal_ema_beta_k(source). No attention output projection is used in the fit.",
        },
        "results": results,
    }
    if args.output_json:
    ...
    for source_name, source_result in result["results"].items():
        ...
        if best:
            print(
                ...
                f"coeffs={[round(x, 6) for x in best['coeffs']]}"
            )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fit per-head sums of causal exponential kernels to DA-V2 attention heads.")
    ...
    parser.add_argument("--basis-betas", default="0,0.25,0.5,0.75,0.9,0.97,0.99")
    parser.add_argument("--ridge", type=float, default=1e-6)
    parser.add_argument("--log-every", type=int, default=4)
    parser.add_argument("--output-json", type=Path, default=Path("eval_outputs/head_exp_combo_fit_vits.json"))
    return parser
